...
def _check_shape(payload):
    ...
    if not isinstance(lines, list) or not lines:
        return _error(
            "VALIDATION_ERROR",
            "'lines' must contain at least one entry",
        )
    ...
    for index, item in enumerate(lines):
        ...
        if not isinstance(item.get("description"), str) or not item["description"]:
            return _error(
                "VALIDATION_ERROR",
                f"lines[{index}].description is required",
            )

        if not isinstance(item.get("unit"), str) or not item["unit"]:
            return _error(
                "VALIDATION_ERROR",
                f"lines[{index}].unit is required",
            )
        ...
        if not isinstance(item.get("tax_code"), str):
            return _error(
                "VALIDATION_ERROR",
                f"lines[{index}].tax_code is required",
            )

        for optional in ("quantity", "unit_price"):
            if (
                item.get(optional) is not None
                and not _is_int(item[optional])
            ):
                return _error(
                    "VALIDATION_ERROR",
                    f"lines[{index}].{optional} must be an integer or null",
                    {"received": item[optional]},
                )
...
def _check_business_rules(payload):
    ...
    unknown = sorted(
        {
            ...
            if item["tax_code"] not in TAX_RATES
        }
    )
    ...
    if unknown:
        return _error(
            ...
            {
                "unknown_tax_codes": unknown,
                "known": sorted(TAX_RATES),
            },
        )
    ...
    expected_subtotal = sum(
        item["amount"] for item in payload["lines"]
    )
    ...
    for item in payload["lines"]:
        subtotal_by_code[item["tax_code"]] = (
            subtotal_by_code.get(item["tax_code"], 0)
            + item["amount"]
        )
...
def _create_invoice(payload):
    ...
    already_registered = any(
        record["partner_code"] == payload["partner_code"]
        and record["invoice_number"] == payload["invoice_number"]
        ...
        if self.path == "/health":
            self._send(
                200,
                {
                    ...
                    "data": {
                        "status": "ok",
                        "registered_invoices": len(_records),
                    },
                    "error": None,
                },
            )
            ...
            tax_codes = [
                {
                    "tax_code": code,
                    "rate": rate,
                    "label": f"Consumption tax {int(rate * 100)}%",
                }
...
def main():
    print(
        f"Mock Accounting API listening on "
